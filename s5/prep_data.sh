...
# 
# The format of the input info file (e.g. metadata.tsv) this script expects (tab-separated columns):
#
# <id>	<filename>	<speaker_id>	<sex>	<age>	<native_language>	<duration>	<sample_rate>	<sentence>	<released>
#
# e.g:
# 12345	bc78cbe8-673f-4250-8fb2-ac63bc8c14ba.wav	2	female	20-29	Icelandic	1.2	48000	Halló heimur.	v1
#
...
. ./utils.sh
. ./prep_filesystem.sh export_filenames_only
...
# Todo maybe? Can this process be parallelized or skipped if already done?
cat $train_data_info_file | tail -n+2 | while IFS=$'\t' read -r utt_id filename speaker_id gender age native_lang length original_sample_rate content version;
do
	# text
...
spinner $!;
#Todo change this spinner to a loading bar as it is more discriptive
...
cut -f2- $lexicon_file | tr ' ' '\n' | LC_ALL=C sort -u > $nonsilence_phones_file;
...
println "Finished running: utils/validate_data_dir.sh";
...
println "Finished running: utils/prepare_lang.sh";
