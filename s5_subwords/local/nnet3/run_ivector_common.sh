...
if [ "$speed_perturb" == "true" ]; then
  if [ $stage -le 1 ]; then
    ...
    utils/data/perturb_data_dir_speed_3way.sh $inputdata data/$code/${train_set}${suffix}
    echo "$0: making MFCC features for low-resolution speed-perturbed data"
    steps/make_mfcc.sh --nj 100 --cmd "$train_cmd --time 2-00" \
    data/$code/${train_set}${suffix} || exit 1
    steps/compute_cmvn_stats.sh data/$code/${train_set}${suffix} || exit 1
    utils/fix_data_dir.sh data/$code/${train_set}${suffix} || exit 1
  fi
  
  if [ $stage -le 2 ] && [ "$generate_alignments" == "true" ]; then
    #obtain the alignment of the perturbed data
    steps/align_fmllr.sh --nj 100 --cmd "$decode_cmd --time 2-00" \
    data/$code/${train_set}${suffix} $langdir $gmm_dir $ali_dir || exit 1
  fi
...
if [ $stage -le 3 ]; then
  ...
  if [ "$speed_perturb" == "true" ]; then
    utils/copy_data_dir.sh data/$code/$train_set data/$code/${train_set}_hires
  else
    utils/copy_data_dir.sh $inputdata data/$code/${train_set}_hires
  fi
  
  # do volume-perturbation on the training data prior to extracting hires
  # features; this helps make trained nnets more invariant to test data volume.
  utils/data/perturb_data_dir_volume.sh data/$code/${train_set}_hires
  ...
  utils/fix_data_dir.sh data/$code/${train_set}_hires;
  ...
  utils/subset_data_dir.sh data/$code/${train_set}_hires 35000 data/$code/${train_set}_35k_hires
  #utils/subset_data_dir.sh data/$code/${train_set}_hires 100000 data/$code/${train_set}_100k_hires
fi
...
if [ $stage -le 5 ]; then
  ...
  data/$code/${train_set}_35k_hires exp/$code/nnet3/pca
fi
...
if [ $stage -le 6 ]; then
  ...
  data/$code/${train_set}_35k_hires 512 exp/$code/nnet3/pca exp/$code/nnet3/diag_ubm
fi
...
if [ $stage -le 7 ]; then
  ...
  data/$code/${train_set}_hires exp/$code/nnet3/diag_ubm exp/$code/nnet3/extractor || exit 1;
fi
...
if [ $stage -le 8 ]; then
  ...
  ivectordir=exp/$code/nnet3/ivectors_${train_set}
  
  # having a larger number of speakers is helpful for generalization, and to
  # handle per-utterance decoding well (iVector starts at zero).
  temp_data_root=${ivectordir}
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
  data/$code/${train_set}_hires ${temp_data_root}/${train_set}_hires_max2
  
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd --time 2-00" --nj 100 \
  ${temp_data_root}/${train_set}_hires_max2 \
  exp/$code/nnet3/extractor $ivectordir
  
  # Also extract iVectors for the test data
  for data_set in dev test; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
    data/$code/${data_set}_hires exp/$code/nnet3/extractor exp/$code/nnet3/ivectors_${data_set} || exit 1;
  done
fi
