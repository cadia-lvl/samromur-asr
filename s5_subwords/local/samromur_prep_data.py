...
def parse_arguments():
    ...
    parser.add_argument("-a", "--audio_dir", type=dir_path, help="The Samromur audio path", )
    parser.add_argument("-m", "--meta_file", type=file_path, help="The Samromur metadata file",)
    parser.add_argument("-o", "--output_dir", type=str, help="Where to place the created files", )
...
def append_to_file(df:pd.DataFrame, audio_dir:str, text, wavscp, utt2spk, spk2gender):
    ...
    for i in df.index:
        ...
        wavscp.write(f"{utt_id} sox - -c1 -esigned -r {df.at[i, 'sample_rate']} -twav - < {Path(audio_dir).joinpath(df.at[i, 'filename'])} |\n")
...
def main():
    ...
    df = pd.read_csv(metadata, sep="\t", index_col="id", dtype="str")
