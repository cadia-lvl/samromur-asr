...
dir=exp/$code/chain/tdnn${affix}${suffix}

train_set=$(basename $inputdata)$suffix
#train_set=train_okt2017$suffix
ali_dir=exp/$code/${gmm}_ali_${train_set}
treedir=exp/$code/chain/${gmm}_tree$suffix # NOTE!
lang=data/$code/lang_chain
...
local/nnet3/run_ivector_common.sh --stage $stage \
                                  ...
                                  $inputdata $testdatadir $langdir $gmm $code|| exit 1;

# See if regular alignments already exist
if [ -f ${ali_dir}/num_jobs ]; then
  n_alijobs=$(cat ${ali_dir}/num_jobs)
else
  n_alijobs=`cat data/$code/${train_set}/utt2spk|cut -d' ' -f2|sort -u|wc -l`
  generate_ali_from_lats=true
  ali_dir=exp/$code/${gmm}_lats$suffix
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $n_alijobs \
                            ...
                            $langdir \
                            exp/$code/${gmm} \
                            exp/$code/${gmm}_lats$suffix
  
  rm exp/$code/${gmm}_lats$suffix/fsts.*.gz # save space
fi
...
if [ $stage -le 13 ]; then
  ...
  --feat.online-ivector-dir exp/$code/nnet3/ivectors_${train_set} \
  ...
  --feat-dir data/$code/${train_set}_hires \
  --tree-dir $treedir \
  --lat-dir exp/$code/${gmm}_lats$suffix \
  --dir $dir  || exit 1;
...
if [ $stage -le 15 ]; then
  ...
  for decode_set in dev test; do
    ...
    (
      num_jobs=$(cat data/$code/${decode_set}_hires/utt2spk|cut -d' ' -f2|sort -u|wc -l)
      #--cmd "$decode_cmd --time 0-06" $iter_opts \
      steps/nnet3/decode.sh --acwt 1.0 \
                            --post-decode-acwt 10.0 \
                            --nj $num_jobs \
                            --online-ivector-dir exp/$code/nnet3/ivectors_${decode_set} \
                            $graph_dir \
                            data/$code/${decode_set}_hires \
                            $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_6g || exit 1;
      
      #--cmd "$decode_cmd" \
      steps/lmrescore_const_arpa.sh $decoding_lang \
                                    $rescoring_lang \
                                    data/$code/${decode_set}_hires \
                                    $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_{6g,10g} || exit 1;
    ) || touch $dir/.error &
  done
  wait
  if [ -f $dir/.error ]; then
...
if [ $zerogram_decoding = true ]; then
  ...
  for decode_set in dev test; do
    (
      ...
      --online-ivector-dir exp/$code/nnet3/ivectors_${decode_set} \
      $dir/graph_zg data/$code/${decode_set}_hires \
...
if [ $calculate_bias = true ]; then
  ...
  num_jobs=`cat data/$code/train-dev_hires/utt2spk|cut -d' ' -f2|sort -u|wc -l`
  steps/nnet3/decode.sh \
  --acwt 1.0 --post-decode-acwt 10.0 \
  --nj $num_jobs --cmd "$decode_cmd --time 0-06" $iter_opts \
  --online-ivector-dir exp/$code/nnet3/ivectors_train-dev \
  $graph_dir data/$code/train-dev_hires \
