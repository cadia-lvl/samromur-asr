...
if [ $stage -le 3 ] && $create_high_mfcc; then
  ...
  for data_set in dev test; do
    ...
    steps/compute_cmvn_stats.sh $testdatadir/${data_set}_hires \
      $log/mfcc/${data_set}_hires \
      $mfccdir/${data_set}_hires;

    utils/validate_data_dir.sh $testdatadir/${data_set}_hires || utils/fix_data_dir.sh $testdatadir/${data_set}_hires 
