...
if [ $stage -le 1 ]; then
  ...
  echo "INFO (run_ivector_common.sh): Stage 1 Done!"
fi
...
#Changed test for eval
  ...
  echo "INFO (run_ivector_common.sh): Stage 2 Done!"
fi
...
if [ $stage -le 3 ]; then
  ...
  echo "INFO (run_ivector_common.sh): Stage 3 Done!"
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "utils/slurm.pl --mem 4G" --num-iters 7 --mllt-iters "2 4 6" \
     --splice-opts "--left-context=3 --right-context=3" \
     3000 10000 $temp_data_root/${train_set}_hires data/lang \
      $gmm_dir exp/nnet3${nnet3_affix}/tri5

    echo "INFO (run_ivector_common.sh): Stage 4 Done!"
fi
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd --mem 8G" --nj 30 \
    --num-frames 700000 \
    --num-threads $num_threads_ubm \
    ${temp_data_root}/${train_set}_sp_hires_subset 512 \
    exp/nnet3${nnet3_affix}/tri5 exp/nnet3${nnet3_affix}/diag_ubm

    echo "INFO (run_ivector_common.sh): Stage 5 Done!"
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd --mem 8G" --nj 10 \
    data/${train_set}_sp_hires exp/nnet3${nnet3_affix}/diag_ubm exp/nnet3${nnet3_affix}/extractor || exit 1;

    echo "INFO (run_ivector_common.sh): Stage 6 Done!"
fi
...
if [ $stage -le 7 ]; then
  ...
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd --mem 8G" --nj $nj \
  ...
  echo "INFO (run_ivector_common.sh): Stage 7 Done!"
fi
...
if [ $stage -le 8 ]; then
  ...
  echo "INFO (run_ivector_common.sh): Stage 8 Done!"
fi

if [ $stage -le 9 ]; then
  echo "$0: making MFCC features for low-resolution speed-perturbed data"
  steps/make_mfcc.sh --nj $nj \
    --cmd "$train_cmd --mem 8G" data/${train_set}_sp
  steps/compute_cmvn_stats.sh data/${train_set}_sp
  echo "$0: fixing input data-dir to remove nonexistent features, in case some "
  echo ".. speed-perturbed segments were too short."
  utils/fix_data_dir.sh data/${train_set}_sp

  echo "INFO (run_ivector_common.sh): Stage 9 Done!"
fi
...
if [ $stage -le 10 ]; then
  ...
  echo "INFO (run_ivector_common.sh): Stage 10 Done!"
fi
...
if [ $stage -le 11 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd --mem 8G" \
         data/${train_set}_sp_comb data/lang $gmm_dir $ali_dir

  echo "INFO (run_ivector_common.sh): Stage 11 Done!"
fi
