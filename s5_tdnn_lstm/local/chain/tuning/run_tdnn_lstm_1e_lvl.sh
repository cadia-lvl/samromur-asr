...
#--------------------------------------------------------------------#
#NOTICE: THIS SCRIPT HAS BEEN MODIFIED BY CARLOS MENA (carlosm@ru.is)
#--------------------------------------------------------------------#
#--------------------------------------------------------------------#
...
echo "INFO (run_tdnn_lstm.sh): Script run_ivector_common.sh Done!"
...
if [ $stage -le 14 ]; then
  ...
  echo "INFO (run_tdnn_lstm.sh): Stage 14 Done!"
fi
...
if [ $stage -le 15 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 30 --cmd "$train_cmd --mem 4G" ${lores_train_data_dir} \
    data/lang $gmm_dir $lat_dir
  rm $lat_dir/fsts.*.gz # save space
  echo "INFO (run_tdnn_lstm.sh): Stage 15 Done!"
fi
...
if [ $stage -le 16 ]; then
  ...
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd "$train_cmd --mem 4G" 4000 ${lores_train_data_dir} data/lang_chain $ali_dir $tree_dir

  echo "INFO (run_tdnn_lstm.sh): Stage 16 Done!"
fi
...
EOF
  ...
  echo "INFO (run_tdnn_lstm.sh): Stage 17 Done!"
fi
...
if [ $stage -le 18 ]; then
 ...
 echo "INFO (run_tdnn_lstm.sh): Stage 18 Done!"
fi
...
if [ $stage -le 19 ]; then
  ...
  echo "INFO (run_tdnn_lstm.sh): Stage 19 Done!"
fi

#--------------------------------------------------------------------#
# Stage 20 - 1 of 3 Decodings
#--------------------------------------------------------------------#
...
if [ $stage -le 20 ]; then
  ...
  echo "INFO (run_tdnn_lstm.sh): Stage 20 Done!"
fi

#--------------------------------------------------------------------#
# Stage 21 - 2 of 3 Decodings
#--------------------------------------------------------------------#
...
if [ $stage -le 21 ]; then
  ...
  echo "INFO (run_tdnn_lstm.sh): Stage 21 Done!"
fi

#--------------------------------------------------------------------#
# Stage 22 - 3 of 3 Decodings
#--------------------------------------------------------------------#
...
if $test_online_decoding && [ $stage -le 22 ]; then
  ...
  echo "INFO (run_tdnn_lstm.sh): Stage 22 Done!"
fi

#--------------------------------------------------------------------#
#END of the Script. Exit with no Error (status 0)
#--------------------------------------------------------------------#
exit 0
#--------------------------------------------------------------------#
