...
# If status not included in metadata
dataset = dict()
for f, ds in [("train.txt", "train"), ("dev.txt", "dev"), ("test.txt", "test")]:
	with open(os.path.join(CORPUS_ROOT, f)) as fset:
		for l in fset.readlines():
			dataset[l.strip()] = ds
