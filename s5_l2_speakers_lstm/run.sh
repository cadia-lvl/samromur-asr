...
corpus_root=./samromur_l2_speakers_data

prondict_orig=metadata/IS_LMs_with_Prons_22.01/models/Pronouncing_Dictionary/ICELANDIC_PRONUNCIATION_DICTIONARY.dic

arpa_lm_3g=metadata/IS_LMs_with_Prons_22.01/models/3-gram_lm/3GRAM_ARPA_MODEL_PRUNED.lm
arpa_lm_4g=metadata/IS_LMs_with_Prons_22.01/models/4-gram_lm/4GRAM_ARPA_MODEL.lm
...
if  [ $current_stage -ge $from_stage ] && [ $current_stage -le $to_stage ]; then
    ...
    python3 local/samromur_l2_speakers_data_prep.py $corpus_root
...
if  [ $current_stage -ge $from_stage ] && [ $current_stage -le $to_stage ]; then
    ...
    local/chain/run_tdnn_lstm_ADAPTED.sh --stage 15 --train-set train --gmm tri3 --nnet3-affix ""
    wait
