...
if [ $stage -le 4 ]; then
    if [ ! -d data/lang ]; then
        ...
        $train_cmd data/lang/log/prep_lang.log local/prep_lang.sh $prondict $localdict data/lang
    fi
fi
...
if [ $stage -le 7 ]; then
    ...
    steps/train_mono.sh --nj $nj_train --cmd "$train_cmd --mem 4G" --boost-silence 1.25 \
      data/train_5k data/lang exp/mono

    echo "Mono alignment. Align train_5k to mono"
    steps/align_si.sh --nj $nj_train --cmd "$train_cmd" data/train_5k data/lang \
...
# Train LDA + MLLT
if [ $stage -le 8 ]; then
    echo "Aligning the train_10 set to tri2"
    steps/align_si.sh --nj $nj_train --cmd "$train_cmd" data/train_10k data/lang exp/tri1 \
      exp/tri1_ali_10k
...
if [ $stage -le 10 ]; then
    ...
    for dir in dev eval; do
        (
            steps/decode_fmllr.sh --config conf/decode.config --nj "$nj_decode"  \
              --cmd "$decode_cmd" exp/tri3/graph data/$dir exp/tri3/decode_$dir;

            steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" data/lang_{3g,4g} data/$dir \
              exp/tri3/decode_$dir exp/tri3/decode_${dir}_rescored
        ) &
    done
    wait
fi
