...
nj=30
train_set=train
test_sets="test test_malromur test_althingi"
...
if [ $stage -le 13 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$align_cmd" ${lores_train_data_dir} \
...
if [ $stage -le 17 ]; then
  ...
  utils/lang/check_phones_compatible.sh \
    data/lang_3g/phones.txt $lang/phones.txt
  utils/mkgraph.sh \
    --self-loop-scale 1.0 data/lang_3g \
    $tree_dir $tree_dir/graph_3g || exit 1;

  utils/lang/check_phones_compatible.sh \
    data/lang_4g/phones.txt $lang/phones.txt
  utils/mkgraph.sh \
    --self-loop-scale 1.0 data/lang_4g \
    $tree_dir $tree_dir/graph_4g || exit 1;
...
if [ $stage -le 18 ]; then
  ...
  for data in $test_sets; do
    (
      ...
      steps/lmrescore.sh \
        --self-loop-scale 1.0 \
        --cmd "$decode_cmd" data/lang_phoneme data/lang_4g data/${data}_hires \
        ${dir}/decode_phoneme_${data_affix} ${dir}/decode_4g_${data_affix} || exit 1
    ) || touch $dir/.error &
  done
  wait
