...
if [ ! $# = 3 ]; then
  ...
  echo ""
  echo "Usage: $0 [options] <input-training-data-dir> <lang-dir> <gmm-name>"
...
mfccdir=$exp/mfcc/${train_set}
   


if [ $stage -le 1 ] && [ "$speed_perturb" == "true" ]; then
echo ===========================================================================
...
echo ============================================================================
  ...
  steps/make_mfcc.sh --nj $nj \
    ...
    $data/$train_set  \
    $log/mfcc/$train_set \
    $exp/mfcc/$train_set || exit 1
                      
  steps/compute_cmvn_stats.sh $exp/mfcc/$train_set || exit 1
  
  utils/fix_data_dir.sh $exp/mfcc/$train_set || exit 1
fi
...
if [ $stage -le 3 ] && $create_high_mfcc; then
  ...
  echo "$0: creating high-resolution MFCC features" train_set
...
if [ $stage -le 7 ]; then
  echo ===========================================================================
  echo "               Training iVecotor extractor			                "
  echo ============================================================================
